...
async function migrateWithoutContent() {
  console.log('🔍 Ejecutando migración sin contenido...');
  ...
  try {
    ...
    for (const file of mdxFiles) {
      try {
        ...
        // Crear datos del artículo SIN el campo content
        const articleData = {
          title: frontmatter.title,
          slug: `${slug}-no-content`, // Usar un slug diferente para evitar conflictos
          excerpt: (frontmatter.description || '').substring(0, 80),
          // NO incluir el campo content
        ...
        console.log(`✅ Artículo creado: ${slug}-no-content`);
        console.log(`📝 Contenido disponible para copiar manualmente:`);
        console.log(`   Longitud: ${content.length} caracteres`);
        console.log(
          `   Primeros 200 caracteres: ${content.substring(0, 200)}...`
        );
        createdCount++;
      } catch (error) {
    ...
    console.log(`\n📋 Próximos pasos:`);
    console.log(`1. Los artículos se crearon sin contenido`);
    console.log(`2. Accede al panel de administración de Strapi`);
    console.log(`3. Edita cada artículo y copia el contenido manualmente`);
    console.log(`4. El contenido está disponible en los logs de arriba`);
  } catch (error) {
    console.error('❌ Error general:', error);
  }
}

migrateWithoutContent();
