...
function fixArticle2025(filePath: string): ArticleFix {
  ...
  try {
    ...
    if (!content.includes("pubDate: '2025-")) {
      return fix; // No es un artículo de 2025
    }
    ...
    for (const line of lines) {
      if (line.includes(':')) {
        ...
        if (key === 'tags') {
          hasTags = true;
          // Verificar si hay múltiples bloques de tags
          if (frontmatterText.match(/tags:\s*\[/g)?.length > 1) {
            hasDuplicateTags = true;
          }
        } else if (key === 'cover') {
          coverUrl = value.replace(/"/g, '').replace(/'/g, '');
        } else if (key === 'author') {
          frontmatter.author = {
            name: '',
            description: '',
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (skipUntilNextField) {
        if (
          ...
          continue; // Saltar líneas de tags duplicados
        }
      }
      ...
      if (line.includes(':')) {
        ...
        if (key === 'tags') {
          // Si hay tags duplicados o no hay tags válidos, reemplazar
          if (hasDuplicateTags || !hasTags) {
            ...
            newFrontmatterLines.push(`  ]`);
            fix.changes.push(`Tags corregidos: ${newTags.join(', ')}`);
            hasChanges = true;

            // Saltar hasta el siguiente campo
...
async function main() {
  console.log('🔧 Corrigiendo específicamente artículos de 2025...\n');
  ...
  for (const filePath of files) {
    const fix = fixArticle2025(filePath);
    if (fix.fixed || fix.errors.length > 0) {
      fixes.push(fix);
  ...
  const result = {
    ...
    fixedArticles: fixes.filter((f) => f.fixed).length,
    failedArticles: fixes.filter((f) => !f.fixed && f.errors.length > 0).length,
    summary: {
      tagsFixed: fixes.filter(
        (f) => f.fixed && f.changes.some((c) => c.includes('Tags'))
      ).length,
      coversFixed: fixes.filter(
        (f) => f.fixed && f.changes.some((c) => c.includes('Cover'))
      ).length,
      errors: fixes.reduce((sum, f) => sum + f.errors.length, 0),
    },
    fixedList: fixes.filter((f) => f.fixed).map((f) => f.filename),
    failedList: fixes
      .filter((f) => !f.fixed && f.errors.length > 0)
      .map((f) => f.filename),
  };

  // Mostrar resultados
  console.log('\n📊 RESULTADOS DE LA CORRECCIÓN ESPECÍFICA\n');
  console.log(`📁 Total de artículos 2025 procesados: ${result.totalArticles}`);
  console.log(`✅ Artículos corregidos: ${result.fixedArticles}`);
  console.log(`❌ Artículos con errores: ${result.failedArticles}\n`);

  console.log('📈 RESUMEN DE CAMBIOS:');
  console.log(`• Tags corregidos: ${result.summary.tagsFixed}`);
  ...
  fs.writeFileSync(
    'fix-2025-specific-report.json',
    JSON.stringify(report, null, 2)
  );
  console.log(
    '📄 Reporte de correcciones específicas guardado en: fix-2025-specific-report.json'
  );
