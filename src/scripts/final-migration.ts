...
function convertContentToBlocks(content: string) {
  // Convertir el contenido MDX a bloques de Strapi usando richtext
  return [
    {
...
async function finalMigration() {
  console.log('🔍 Ejecutando migración final...');
    ...
    for (const file of mdxFiles) {
      try {
        ...
        const articleData = {
          ...
          slug: `${slug}-final`, // Usar un slug diferente para evitar conflictos
        ...
        console.log(`✅ Artículo creado: ${slug}-final`);
        createdCount++;
      } catch (error) {
