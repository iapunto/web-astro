...
export default function ClientBlogLoader({ fallbackArticles }: Props) {
  ...
  useEffect(() => {
    const loadArticles = async () => {
      try {
        console.log('🔄 [CLIENT] Cargando artículos desde Strapi (client-side)...');
        
        // Fetch directo a Strapi desde el navegador del usuario
        const response = await fetch('https://strapi.iapunto.com/api/articles?populate=*&sort[0]=publishedAt:desc&pagination[pageSize]=100', {
          headers: {
  ...
  return (
    <div style={{ display: 'none' }} data-articles={JSON.stringify(articles)} data-loading={loading} data-error={error}>
