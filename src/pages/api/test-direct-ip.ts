...
export async function GET() {
  ...
  const tests = [
    {
      name: 'Dominio HTTPS',
      url: 'https://strapi.iapunto.com/api/articles?pagination[pageSize]=1',
    },
    {
      name: 'IP directa HTTPS',
      url: 'https://190.146.4.75/api/articles?pagination[pageSize]=1',
      headers: { Host: 'strapi.iapunto.com' },
    },
    {
      name: 'IP directa HTTP',
      url: 'http://190.146.4.75:1337/api/articles?pagination[pageSize]=1',
      headers: { Host: 'strapi.iapunto.com' },
    },
    {
      name: 'IP sin Host header',
      url: 'https://190.146.4.75/api/articles?pagination[pageSize]=1',
    },
  ];
  ...
  for (const test of tests) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `✅ [TEST-IP] ¡FUNCIONA! ${test.name} - Status: ${response.status}, Tiempo: ${result.time}ms`
        );
      } else {
