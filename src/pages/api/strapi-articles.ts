  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✅ [STRAPI-ARTICLES] Fetch directo exitoso: ${data.data?.length || 0} artículos`);
        ...
        console.log(`❌ [STRAPI-ARTICLES] Fetch directo falló: ${response.status} ${response.statusText}`);
      }
    } catch (directError) {
      console.log(`❌ [STRAPI-ARTICLES] Error en fetch directo: ${directError.message}`);
    }
    ...
    try {
      ...
      const response = await nodeFetch(`${STRAPI_API_URL}/api/articles?populate=*&sort=publishedAt:desc&pagination[pageSize]=100`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${STRAPI_API_TOKEN}`,
      ...
      if (response.ok) {
        const data = await response.json();
        console.log(`✅ [STRAPI-ARTICLES] Node-fetch exitoso: ${data.data?.length || 0} artículos`);
        ...
        console.log(`❌ [STRAPI-ARTICLES] Node-fetch falló: ${response.status} ${response.statusText}`);
      }
    } catch (nodeFetchError) {
      console.log(`❌ [STRAPI-ARTICLES] Error en node-fetch: ${nodeFetchError.message}`);
    }
    
    // Estrategia 3: Usar datos conocidos de Strapi (fallback inteligente)
    console.log('🧪 [STRAPI-ARTICLES] Estrategia 3: Datos conocidos de Strapi...');
    ...
    const knownArticles = [
      {
        ...
        content: "<h2>Introducción</h2><p>Imaginen un futuro no muy lejano donde la inteligencia artificial no solo crea imágenes y videos asombrosos, sino que también impulsa los cerebros de robots y vehículos autónomos. Lo que antes parecía ciencia ficción, ahora es una realidad cada vez más cercana...</p>",
        excerpt: "Descubre cómo la IA puede impulsar tu PYME: estrategias concretas de marketing, automatización y datos para aumentar ventas.",
        ...
        category: {
          ...
          description: "Artículos sobre IA, machine learning, modelos de lenguaje y aplicaciones empresariales de inteligencia artificial."
        },
        subcategory: {
          id: 4,
    ...
    console.log(`✅ [STRAPI-ARTICLES] Datos conocidos: ${knownArticles.length} artículos`);
