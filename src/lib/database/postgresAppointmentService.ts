import { getPool, initializeDatabase, Appointment, EmailTemplate, EmailLog, SystemSettings } from './postgresSchema.js';
...
export class PostgresAppointmentService {
  ...
  async updateAppointment(id: string, updates: Partial<Appointment>): Promise<Appointment> {
    ...
    try {
      ...
      const values = [id, ...fields.map(field => updates[field as keyof Appointment])];
  ...
  async getEmailTemplate(type: string): Promise<EmailTemplate | null> {
    ...
    try {
      ...
      return result.rows.length > 0 ? this.mapRowToEmailTemplate(result.rows[0]) : null;
    } finally {
      client.release();
    }
  }
  ...
  async createEmailTemplate(template: Omit<EmailTemplate, 'id' | 'createdAt' | 'updatedAt'>): Promise<EmailTemplate> {
    ...
    try {
      const result = await client.query(`
        ...
        VALUES ($1, $2, $3, $4, $5)
        RETURNING *
      `, [template.name, template.subject, template.body, template.type, template.isActive]);
  ...
  async updateEmailTemplate(id: string, updates: Partial<EmailTemplate>): Promise<EmailTemplate> {
    ...
    try {
      ...
      const values = [id, ...fields.map(field => updates[field as keyof EmailTemplate])];
  ...
  async createEmailLog(log: Omit<EmailLog, 'id' | 'createdAt'>): Promise<EmailLog> {
    const client = await this.getClient();
  ...
  async getEmailLogsByAppointmentId(appointmentId: string): Promise<EmailLog[]> {
    const client = await this.getClient();
